         ...
         isset($_POST['current_cover'])) {
            ...
            if(!empty($_FILES['book_cover']['name'])){
                ...
                if($book_cover['status'] == "error"){
                    $em = $book_cover['data'];
                    header("Location: ../edit_books.ph?error=$em&id=$id");
                    exit;
                } else {
                    $c_p_book_cover = "../descarcate/web/$current_cover";
                    unlink($c_p_book_cover);
                    $book_cover_URL=$book_cover['data'];
                    $sql = "UPDATE carti SET Titlu=?,
                                             id_autor=?,
                                             id_categorie=?,
                                             Numarul_de_pagini=?,
                                             Editura=?,
                ...
                if($res){
                    ...
                    header("Location: ../edit_books.php?success=$sm&id=$id");
                    exit;
                }else{
                    $em="Unknown error occured !";
                    header("Location: ../edit_books.php?error=$em&id=$id");
                    exit;
                    }
                }
